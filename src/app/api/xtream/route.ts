...
export async function POST(request: Request) {
  ...
  if (!upstream.ok) {
    // 404 needs its own message, but not a confident diagnosis. A portal that
    // serves the API perfectly well still answers 404 when it dislikes
    // something about the request — several panels use it for bad credentials
